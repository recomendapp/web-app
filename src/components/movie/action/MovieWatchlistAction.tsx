...
const like = (userId: string, movieId: number) =>
  fetch(`/api/user/${userId}/movie/${movieId}/like`).then((res) => res.json());
const unlike = (userId: string, movieId: number) =>
  fetch(`/api/user/${userId}/movie/${movieId}/unlike`).then((res) =>
    res.json()
  );
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            {isLoading ? (
              ...
              <Bookmark
                className={
                  isWatchlisted ? 'fill-accent-1-foreground' : 'transparent'
                }
              />
            )}
          </Button>
        ...
        <TooltipContent side="bottom">
          {isWatched ? <p>Envie de le revoir</p> : <p>Envie de le voir</p>}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
