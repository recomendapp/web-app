...
interface DataTableProps {
  ...
  playlist_item: { node: PlaylistItemFragment }[];
  setPlaylist_item: React.Dispatch<React.SetStateAction<{ node: PlaylistItemFragment }[]>>;
...
export default function PlaylistTable({
  ...
  const items = React.useMemo<UniqueIdentifier[]>(() => playlist_item?.map(({ node }) => node.id), [playlist_item]);
  ...
  const selectedRow = React.useMemo(() => {
    ...
    const row = rowData.find(({ original }) => original?.node.id === activeId);
    // prepareRow(row);
    return row;
  }, [activeId, rowData]);
...
const TableContainer = ({
  ...
  table: TableType<{ node: PlaylistItemFragment; }>;
        ...
        {table.getRowModel().rows?.length ? (
          <SortableContext items={items} strategy={verticalListSortingStrategy}>
            {table.getRowModel().rows.map((row) => (
              <DraggableTableRow key={row.original.node.id} row={row} />
            ))}
          </SortableContext>
        ) : (
          <TableRow>
...
const DraggableTableRow = ({ row } : { row: Row<{ node: PlaylistItemFragment }>}) => {
  const {
    attributes,
    listeners,
    transform,
...
export const StaticTableRow = ({ row } : { row:  Row<{ node: PlaylistItemFragment; }> | null | undefined}) => {
  return (
