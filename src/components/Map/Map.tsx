...
import ThrillerIcon from '@/components/Map/Icons/thriller.svg';
...
import MapPopup from './OLD/popup/MapPopup';

// TMDB FUNCTION
import { Icons } from '@/components/icons';
// import MapFilter from './mapfilters/MapFiltersOLD';
import { MapFilters } from './OLD/mapfilters/mapfilters';
import { useLocale } from 'next-intl';
import { MapSearchbar } from './OLD/MapSearchbar';
...
const zones = mapZones.features.map((zone) => zone.properties.name);
...
export function Map() {
  ...
  function makeSvgImage(size: number, color: string = '#fff', padding: number = 0) {
    const htmlImage = new Image(size, size);
    const svgString = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}">
        <circle cx="${size / 2}" cy="${size / 2}" r="${size / 2 - padding}" fill="${color}" />
      </svg>
    `;
    htmlImage.src = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(svgString);
    htmlImage.onload = () => {
       map.current.addImage(`circle-${size}-${color}-${padding}`, htmlImage);
    };
  }
    ...
    map.current.on('load', () => {
      ...
      environment_items.forEach((item) => {
        map.current.loadImage(
          `map/environment-items/${item.name}.png`,
          (error: any, image: any) => {
            if (error) throw error;
            map.current.addImage(`envItems_${item.name}`, image);
          }
        );
      });
      ...
      // GRASS
      makeSvgImage(2, '#57523B', 0);
      makeSvgImage(8, '#57523B', 2);
      map.current.addLayer({
        id: 'grass',
        type: 'fill',
        source: 'grass',
        paint: {
          // change circle radius to match the current zoom level
          'fill-pattern': [
            "step",
            ["zoom"],
            "circle-2-#57523B-0",
      ...
      map.current.addLayer({
        ...
        paint: {
          ...
          'line-width': {
            ...
            base: 2,
            stops: [
              [0, 0.75 * Math.pow(2, 0 - viewport.baseZoom)],
              [24, 0.75 * Math.pow(2, 24 - viewport.baseZoom)],
            ],
          },
        },
      });
      ...
      map.current.addLayer({
        ...
        layout: {
          'icon-image': [
            'match',
            ['get', 'Type', ['properties']],
            ...environment_items.flatMap(({ name }) => [name, `envItems_${name}`]),
            'none',
          ],
          // 'icon-size': {
          //   type: 'exponential',
      ...
      'icon-size': [
        ...
        0, 
          ["match", ['get', 'Type'],
              ...environment_items.flatMap(({ name, size }) => [
                name,
                0.05 * Math.pow(2, 0 - viewport.baseZoom),
              ]),
              0.05 * Math.pow(2, 0 - viewport.baseZoom),
      ...
      zones.forEach((name) => {
        ...
        const path = `map/zones/${name}.svg`;
        img.src = path;
        img.onload = () => {
          map.current.addImage(`zone_${name}`, img);
        };
      });
      ...
      map.current.addLayer({
        ...
        layout: {
          'icon-image': [
            'match',
            ['get', 'name', ['properties']],
            ...zones.flatMap((name) => [name, `zone_${name}`]),
            'none',
          ],
          'icon-size': {
            type: 'exponential',
            base: 2,
            stops: [
              [0, 0.04 * Math.pow(2, 0 - viewport.baseZoom)],
              [24, 0.04 * Math.pow(2, 24 - viewport.baseZoom)],
            ],
          },
      ...
      data.genres.forEach(({ id }) => {
        ...
        const path = `map/genres/${id}.svg`;
        img.src = path;
        img.onload = () => {
          map.current.addImage(`genre_${id}`, img);
        };
      });
      ...
      map.current.addLayer({
        ...
        layout: {
          ...
          'text-size': {
            ...
            base: 2,
            stops: [
              [0, 1.5 * Math.pow(2, 0 - viewport.baseZoom)],
              [24, 1.5 * Math.pow(2, 24 - viewport.baseZoom)],
            ],
          },
          ...
          'icon-image': [
            'match',
            ['at', 0, ['get', 'genres']],
            ...data.genres.flatMap(({ id }) => [id, `genre_${id}`]),
            'none',
          ],
          'icon-size': {
            type: 'exponential',
            base: 2,
            stops: [
              [0, 0.004 * Math.pow(2, 0 - viewport.baseZoom)],
              [24, 0.004 * Math.pow(2, 24 - viewport.baseZoom)],
            ],
          },
        },
        paint: {
  ...
  useEffect(() => {
     map.current.on('click', 'movies', (e: any) => {
      const movieId = e.features[0].properties.id as number;
      setMovieId(movieId);
    });

    // Change the cursor to a pointer when the mouse is over the movies layer.
    ...
    return () => {
      map.current.off('click', 'movies');
      map.current.off('mouseenter', 'movies');
      map.current.off('mouseleave', 'movies');
    };
  }, [map, setMovieId]);
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-black">
      ...
      {/* MAP CONTAINER */}
      <div ref={mapContainer} className="map h-full w-full" />
      <Interface />
    </div>
  );
}
