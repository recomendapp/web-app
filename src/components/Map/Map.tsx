  ...
  useEffect(() => {
    map.current.on('load', () => {
      ...
      environment_items.forEach((imagePath) => {
        ...
        map.current.loadImage(`map/environment-items/${imagePath}`, (error: any, image: any) => {
          if (error) throw error;
      ...
      environment_symbols.forEach((imagePath) => {
        const name = imagePath.replace(/\.[^/.]+$/, '');
        map.current.loadImage(`map/environment-symbols/${imagePath}`, (error: any, image: any) => {
          if (error) throw error;
      ...
      buildings.forEach((imagePath) => {
        const name = imagePath.replace(/\.[^/.]+$/, '');
        map.current.loadImage(`map/buildings/${imagePath}`, (error: any, image: any) => {
          if (error) throw error;
      ...
      map.current.addLayer({
        ...
        'layout': {
            'icon-image': [
              'match',
              ['get', 'Type', ['properties']],
              'Potager', 'envItems_bush',
              'Cactus 1', 'envItems_cactus_1',
              'Cactus 2', 'envItems_cactus_2',
              'Cratere', 'envItems_crater',
              'Dune 1', 'envItems_dune_1',
              'Sapin', 'envItems_fir',
              'Meule de foin', 'envItems_haystack',
              'Montagne 1', 'envItems_mountain_1',
              'Palmier', 'envItems_palm',
              'Parasol', 'envItems_parasol',
              'Roche 1', 'envItems_rock_1',
              'Roche 2', 'envItems_rock_2',
              'Massif rocheux 1', 'envItems_rock_mass_1',
              'Massif rocheux 2', 'envItems_rock_mass_2',
              'Aileron requin', 'envItems_shark',
              'Fumee 1', 'envItems_smoke_1',
              'Fumee 2', 'envItems_smoke_2',
              'Bonhomme de neige', 'envItems_snowman',
              'Baleine', 'envItems_whale',
              'Arbre', 'envItems_tree',
              'none'
            ],
            'icon-size': {
      ...
      map.current.addLayer({
        ...
        'layout': {
            'icon-image': [
              'match',
              ['get', 'Nom', ['properties']],
              'Porte avion', 'buildings_aircraft_carrier',
              'Dirigeable', 'buildings_airship',
              'Arène', 'buildings_arena',
              'premier contact', 'buildings_arrival',
              'Caserne', 'buildings_barracks',
              'Grande roue', 'buildings_big_wheel',
              'Sphère bioclimatique', 'buildings_bioclimatic_sphere',
              'Immeuble 1', 'buildings_building_1',
              'Immeuble 2', 'buildings_building_2',
              'Immeuble 3', 'buildings_building_3',
              'Immeuble 4', 'buildings_building_4',
              'Manège', 'buildings_carousel',
              'Chateau 1', 'buildings_castle_1',
              'Chateau 2', 'buildings_castle_2',
              'Chateau 3', 'buildings_castle_3',
              'Eglise', 'buildings_church',
              'Cirque', 'buildings_circus',
              'Immeuble 1 Ville', 'buildings_city_building_1',
              'Immeuble 2 Ville', 'buildings_city_building_2',
              'Immeuble 3 Ville', 'buildings_city_building_3',
              'Immeuble 4 Ville', 'buildings_city_building_4',
              'Tour controle', 'buildings_control_tower',
              'Maison campagne 1', 'buildings_country_house_1',
              'Maison campagne 2', 'buildings_country_house_2',
              'Maison campagne 3', 'buildings_country_house_3',
              'Tour eiffel', 'buildings_eiffel_tower',
              'Usine', 'buildings_factory',
              'Ferme', 'buildings_farm',
              'Eolienne ferme', 'buildings_farm_wind_turbine',
              'Tour dysto 1', 'buildings_futuristic_tower_1',
              'Tour dysto 2', 'buildings_futuristic_tower_2',
              'Grand Budapest Hotel', 'buildings_grand_budapest_hotel',
              'Hopital', 'buildings_hospital',
              'Igloo', 'buildings_igloo',
              'Maison jungle', 'buildings_jungle_house',
              'Panneau Las Vegas', 'buildings_las_vegas',
              'Phare', 'buildings_lighthouse',
              'HLM', 'buildings_low_rent_housing',
              'Manoir', 'buildings_manor',
              'Moulin', 'buildings_mill',
              'Maison modeste', 'buildings_modest_house',
              'Monastère', 'buildings_monastery',
              'Motel', 'buildings_motel',
              'Cinema', 'buildings_movie_theater',
              'Forage pétrole', 'buildings_oil_drilling',
              'Palais', 'buildings_palace',
              'Bateau pirate', 'buildings_pirate_ship',
              'Grue Port', 'buildings_port_crane',
              'Tour prison', 'buildings_prison',
              'Hopital psychiatrique', 'buildings_psychiatric_hospital',
              'Antenne radio', 'buildings_radio_antenna',
              'Bateau radio', 'buildings_radio_ship',
              'Restaurant', 'buildings_restaurant',
              'Lancement fusée', 'buildings_rocket_launch',
              'Ruine 1', 'buildings_ruin_1',
              'Ruine 2', 'buildings_ruin_2',
              'Ruine 3', 'buildings_ruin_3',
              'Saloon', 'buildings_saloon',
              'Ecole', 'buildings_school',
              'Maison coloniale', 'buildings_settlers_house',
              'Ferme silot', 'buildings_silo',
              'Stade', 'buildings_stadium',
              'Scène', 'buildings_stage',
              'Statue liberté', 'buildings_statue_of_liberty',
              'Tour Las Vegas', 'buildings_stratosphere_tower',
              'Pavillon 1', 'buildings_suburban_house_1',
              'Pavillon 2', 'buildings_suburban_house_2',
              'Supermarché', 'buildings_supermarket',
              'Tour 1', 'buildings_tower_1',
              'Tour 2', 'buildings_tower_2',
              'Station train', 'buildings_train_station',
              'Tunnel', 'buildings_tunnel',
              'Villa', 'buildings_villa',
              'Maison blanche', 'buildings_white_house',
              'Case africaine', 'buildings_yurt',
              'none',
            ],
            'icon-size': {
              "type": "exponential",
