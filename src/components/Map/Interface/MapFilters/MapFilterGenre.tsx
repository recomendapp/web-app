			...
			<div className="">
				{data?.genres.map((genre) => (
					<Button
						key={genre.id}
						variant="muted"
						// variant={filters.genres.value.includes(genre.id) ? 'accent-1' : 'accent-1-enabled'}
						onClick={() => {
						...
						className={`
							...
							${filters.genres.value.includes(genre.id) && 'bg-accent-1 text-primary hover:bg-accent-1'}
						`}
					>
						{genre.name}
					</Button>
