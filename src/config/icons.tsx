import { cn } from '@/lib/utils';
import { Cross2Icon } from '@radix-ui/react-icons';
import {
  AlertCircleIcon,
  AlertTriangle,
  ArchiveIcon,
  ArchiveRestoreIcon,
  ArrowDownIcon,
  ArrowDownNarrowWideIcon,
  ArrowLeftIcon,
  ArrowRightIcon,
  ArrowUpIcon,
  ArrowUpNarrowWideIcon,
  BellIcon,
  BookmarkIcon,
  CheckIcon,
  ChevronLeft,
  ChevronRight,
  ClapperboardIcon,
  ClockIcon,
  CompassIcon,
  CopyIcon,
  CreditCard,
  EyeIcon,
  File,
  FileText,
  FilterIcon,
  HeartIcon,
  HelpCircle,
  HomeIcon,
  Image,
  InfoIcon,
  Laptop,
  Link,
  ListPlus,
  ListVideoIcon,
  Loader2,
  LockIcon,
  LogOutIcon,
  LucideIcon,
  LucideProps,
  MinusIcon,
  Moon,
  MoreHorizontalIcon,
  PenIcon,
  Pizza,
  Plus,
  ScaleIcon,
  SearchIcon,
  SendIcon,
  Settings,
  SignatureIcon,
  SparklesIcon,
  StarIcon,
  StoreIcon,
  SunMedium,
  TextIcon,
  Trash,
  Twitter,
  User,
  UsersIcon,
  X,
  ZapIcon,
} from 'lucide-react';

export type Icon = LucideIcon;

export const Icons = {
  logo: (props: LucideProps) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 541.5 541.5" {...props}>
      <rect width="256" height="256" fill="none" />
      
      <line
        x1="208"
        y1="128"
        x2="128"
        y2="208"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="16"
      />
      <line
        x1="192"
        y1="40"
        x2="40"
        y2="192"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="16"
      />
    </svg>
  ),
  site: {
    logo: (props: LucideProps) => (
      <svg
        version="1.1"
        id="Calque_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        x="0px"
        y="0px"
        viewBox="0 0 2000 317.6"
        xmlSpace="preserve"
        {...props}
      >
        <ellipse transform="matrix(0.7071 -0.7071 0.7071 0.7071 -104.3122 255.5879)" className="st0" cx="256.4" cy="253.7" rx="63.3" ry="63.3"/>
        <path className="st0" d="M208.6,105.3c11.6,13.4,28.7,21.9,47.7,21.9c36,0,64.9-30,63.2-66.3c-1.5-33-29.2-59.7-62.2-60.3l0,0
          c-18.9-0.3-36,7.7-47.7,20.7S181.5,42,164.1,42h-5.2c-17.5,0-33.8-7.8-45.6-20.7C99.9,6.6,79.6-1.8,57.5,1.3
          C29.9,5.1,7.7,27.3,3.9,54.8C0.8,77,9.2,97.2,24,110.6c12.9,11.8,20.7,28.1,20.7,45.6v5.2c0,17.5-7.8,33.8-20.7,45.6
          c-13,11.8-21,28.9-20.7,47.8c0.5,33,27.2,60.7,60.2,62.2c36.4,1.7,66.3-27.2,66.3-63.2c0-19.1-8.5-36.2-21.9-47.7
          c-12.9-11.1-19.6-27.7-19.6-44.7v-3.7c0-17.5,6.9-34.7,20-46.4c2-1.7,3.8-3.5,5.5-5.5c11.6-13.1,28.8-20,46.4-20h3.7
          C180.9,85.7,197.4,92.5,208.6,105.3z"/>
        <path className="st0" d="M679.4,249.2c-1.1-4-1.7-10.2-2-18.4l-1.1-29c-0.5-12.6-2.9-21.7-7.2-27.2c-4.3-5.6-10.6-10.1-19-13.5
          c9.6-5.1,17.1-11.7,22.5-19.9c5.3-8.2,8-18.8,8-31.8c0-24.8-10.2-41.9-30.5-51.1c-10.7-4.8-24.3-7.2-40.8-7.2H509.3v216.2h29.3
          v-92.7h66.6c11.9,0,20.7,1.4,26.3,4c9.8,4.9,15.1,14.3,15.8,28.4l1.8,36c0.4,8.5,0.9,14.1,1.4,17.1c0.6,3,1.4,5.4,2.4,7.2h35.9
          v-4.9C684.3,260.5,681.2,256.1,679.4,249.2z M638.8,141.9c-7.9,5.5-18.8,8.2-32.6,8.2v0h-67.7V76.2H610c10,0,18,1.4,24,4.4
          c11.2,5.6,16.8,16.1,16.8,31.5C650.8,126.5,646.8,136.4,638.8,141.9z"/>
        <polygon className="st0" points="509.3,51.1 509.3,51.1 509.3,51.1 	"/>
        <path className="st0" d="M852.5,162.2c-1.8-10.6-5.3-19.9-10.6-27.8c-5.5-8.3-13.5-15.1-24-20.4h0c-10.4-5.3-21.3-7.9-32.6-7.7
          c-22.1,0-40.1,7.9-53.8,23.7c-13.7,15.8-20.6,36-20.6,61.6s6.8,45.7,20.5,59.9c13.7,14.3,30.3,21.4,49.9,21.4
          c8.1,0,15.3-0.8,21.7-2.4c11.9-2.8,21.9-8,29.9-15.9c4.8-4.5,9.1-10.4,13-17.5c3.9-7.2,6.2-13.6,6.8-19.4h-26.1
          c-2,7-5.3,13.1-10,18.4c-8.2,9.1-19.2,13.7-33.1,13.7c-14.8,0-25.7-4.9-32.8-14.5c-7.1-9.6-10.8-22.5-11.4-38.5h115.5
          C854.9,181,854.1,169.5,852.5,162.2z M753.2,142.5c8.2-8.8,18.5-13.2,30.6-13.2c17,0,29.1,6.4,36.4,19.1c3.9,6.8,6.4,16,7.5,27.5
          l-87.5,0.1C740.6,162.5,745,151.3,753.2,142.5z"/>
        <path className="st0" d="M948.2,128.8c12.3,0,21.3,3.2,26.9,9.6c5.6,6.4,9.2,14.5,10.8,24.2h25.7c-2.2-21.1-8.9-35.9-20.1-44.6v0
          c-11.1-8.7-25.5-13-43.3-13c-20.8,0-37.9,7.7-51.3,23.1c-13.5,15.4-20.2,37-20.2,64.6c0,22.6,6.1,41.4,18.4,56.5
          c12.3,15,28.7,22.6,49.3,22.6c18.3,0,33.4-4.9,45.2-14.5c11.8-9.6,19.1-24.9,22.1-45.8h-25.7c-2.5,11.7-6.9,20.9-13.3,27.6
          c-6.4,6.8-15.2,10.1-26.1,10.1c-14.4,0-24.8-5.4-31.5-16.2c-6.6-10.8-10-24.2-10-39.9s2.4-28.9,7.1-39.6
          C919.3,137,931.4,128.8,948.2,128.8z"/>
        <path className="st0" d="M1153.2,125.4c-14-13.5-31.2-20.4-51.4-20.4c-22.7,0-40.7,7.9-54.2,23.6c-13.5,15.7-20.2,36.8-20.2,63.3
          c0,24.8,6.4,44.4,19.1,59.1c12.7,14.6,30,22,51.7,22c26,0,45.2-8.3,57.5-24.9c12.3-16.5,18.5-36.7,18.5-63
          C1174.3,158.9,1167.3,138.9,1153.2,125.4L1153.2,125.4z M1136.8,230.3c-6.5,13.3-18.5,20-36.2,19.9c-16,0-27.6-5.6-35-16.8
          c-7.4-11.2-11-24.5-11-42c0-17.5,3.7-32.4,11-44.7c7.4-12.3,19.1-18.5,35.3-18.5c18.3,0,31.1,7.3,38.5,21.8
          c4.7,9.3,7.1,21.2,7.1,35.9C1146.6,202.2,1143.3,217,1136.8,230.3z"/>
        <path className="st0" d="M1368.4,106.1c-9.9,0-18.8,2-26.8,6.1c-7.9,4.1-14.8,10.3-20.7,18.8c-3.5-6.9-7.4-12.1-11.8-15.6
          c-7.7-6.2-17.8-9.3-30.3-9.3c-11.1,0-21,3-29.7,9c-5.1,3.5-10.8,9.2-17.1,16.9v-22.4h-26.2l0,0l0.1,157.6h26.5v-83.6
          c0-20.3,4.1-34.3,12.3-42.1c8.2-7.7,17.3-11.6,27.3-11.6c11,0,18.5,3.7,22.5,11c2.6,4.9,3.8,12.7,3.8,23.4v102.7h26.9v-91.7
          c0-16.3,3.7-27.9,11.2-34.9c7.5-7,16.5-10.4,26.9-10.4c7.6,0,14,2,19.2,5.9c5.3,3.9,7.9,11.2,7.9,21.7v109.5h27.5V162.4
          c0-14-2-25.1-6.2-33.3C1403.9,113.8,1389.6,106.1,1368.4,106.1z"/>
        <path className="st0" d="M1589.5,162.2c-1.8-10.6-5.3-19.9-10.6-27.8c-5.5-8.3-13.5-15.1-24-20.4h0c-10.5-5.3-21.3-7.9-32.6-7.7
          c-22.1,0-40.1,7.9-53.8,23.7c-13.7,15.8-20.6,36-20.6,61.6s6.8,45.7,20.5,59.9c13.7,14.3,30.3,21.4,49.9,21.4
          c8.1,0,15.3-0.8,21.7-2.4c11.9-2.8,21.9-8,29.9-15.9c4.8-4.5,9.1-10.4,13-17.5c3.9-7.2,6.2-13.6,6.8-19.4h-26.1
          c-2,7-5.3,13.1-10,18.4c-8.2,9.1-19.2,13.7-33.1,13.7c-14.8,0-25.7-4.9-32.8-14.5c-7.1-9.6-10.8-22.5-11.4-38.5h115.5
          C1591.8,181,1591.1,169.5,1589.5,162.2z M1490.2,142.5c8.2-8.8,18.5-13.2,30.6-13.2c17,0,29.1,6.4,36.4,19.1
          c3.9,6.8,6.4,16,7.5,27.5l-87.5,0.1C1477.6,162.5,1482,151.3,1490.2,142.5z"/>
        <path className="st0" d="M1701.1,106.1c-10.2,0-19.5,2-27.8,6c-8.3,4.1-16.2,10.7-23.7,19.9v-22.4h-25.1l0.1,157.6h26.5v-82.5
          c0-10.3,0.7-18.3,2-23.8c1.3-5.5,4.2-10.9,8.6-16.1c5.5-6.4,11.8-10.8,19.1-12.9c4-1.2,9.1-1.9,15.4-1.9c12.3,0,20.9,4.9,25.6,14.7
          c2.8,5.9,4.3,13.7,4.3,23.2v99.3h26.9V166.1c0-15.9-2.2-28.1-6.5-36.6C1738.6,113.9,1723.5,106.1,1701.1,106.1z"/>
        <polygon className="st0" points="1624.4,109.7 1624.4,109.7 1624.4,109.6 	"/>
        <path className="st0" d="M1895,129.9c-6-7.4-11.3-12.5-16-15.4l0,0c-8.1-5.1-18.3-7.7-30.4-7.5c-21.7,0-38.5,8.4-50.2,25.2
          c-11.8,16.8-17.6,36.7-17.6,59.7c0,24.6,6.4,44.2,19.3,59c12.9,14.8,28,22.1,45.6,22.1c10.9,0,20.6-2.2,29-6.5
          c8.5-4.3,15.8-11.4,21.9-21.1v21.9h23.8V50.4H1895V129.9z M1884.2,233.9c-7.9,10.6-18,15.8-30.3,15.8c-15.8,0-27.3-5.7-34.4-17.1
          l0,0c-7.2-11.4-10.7-25.5-10.8-42.4c0-19.8,4.1-34.9,12.4-45c8.3-10.2,19.1-15.3,32.4-15.3c12,0,22.1,4.8,30.2,14.4
          c8.1,9.6,12.2,24.6,12.2,44.3S1892.1,223.4,1884.2,233.9z"/>
        <path className="st0" d="M1981.4,235.2c-8.5,0-15.4,6.9-15.4,15.4v1.3c0,8.5,6.9,15.4,15.4,15.4c8.5,0,15.4-6.9,15.4-15.4v-1.3
          C1996.7,242.1,1989.8,235.2,1981.4,235.2z"/>
      </svg>
    ),
    icon: (props: LucideProps) => (
        <svg
          version="1.1"
          id="recomend-icon"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          x="0px" y="0px"
          viewBox="0 0 541.5 541.5"
          xmlSpace="preserve"
          {...props}
        >
          <circle
            className="st0"
            cx="433.8"
            cy="433.2"
            r="108.3"
          />
          <path className="st0" d="M435.5,0c-32.4-0.5-61.5,13.3-81.7,35.4c-20.1,22.1-48.1,35.5-78,35.5h-8.9c-29.9,0-57.9-13.4-78-35.5
            C166,10.2,131.3-4.2,93.5,1.1C46.3,7.6,8.2,45.7,1.7,92.9c-5.2,37.8,9.1,72.5,34.4,95.4c22.1,20.1,35.5,48.1,35.5,78v8.9
            c0,29.9-13.4,57.9-35.5,78c-22.1,20.1-35.9,49.3-35.4,81.6c0.8,56.5,46.6,103.9,103,106.5c62.2,2.9,113.6-46.6,113.6-108.2
            c0-32.7-14.5-61.9-37.4-81.7c-22-19-33.6-47.4-33.6-76.5v-6.3c0-30.1,11.8-59.4,34.3-79.4c3.3-2.9,6.5-6.1,9.4-9.4
            c19.9-22.5,49.3-34.3,79.4-34.3h6.3c29.1,0,57.4,11.6,76.5,33.6c19.8,22.9,49.1,37.4,81.7,37.4c61.5,0,111.1-51.3,108.2-113.5
            C539.4,46.7,492,0.9,435.5,0L435.5,0z"/>
        </svg>
    ),
  },
  premium: (props: LucideProps) => (
    <svg
    version="1.1"
    id="premium-icon"
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    x="0px"
    y="0px"
    viewBox="0 0 105.1 105.1"
    xmlSpace="preserve" 
    {...props}
    >
    <path d="M73.9,75.8l1.8-1.8c3.1-3.1,7.4-4.8,11.8-4.6c5,0.2,10.1-1.8,13.7-6c5.2-6.2,5.2-15.6-0.1-21.7
      c-3.6-4.2-8.8-6.2-13.8-5.9c-4.5,0.3-9-1.3-12.2-4.6L73.9,30c-3.2-3.2-4.8-7.6-4.6-12.2c0.3-5.4-1.9-10.8-6.7-14.5
      c-5.9-4.5-14.3-4.5-20.2,0c-4.8,3.6-7,9.1-6.7,14.5c0.3,4.5-1.3,9-4.6,12.2L30,31.2c-3.2,3.2-7.6,4.8-12.2,4.6
      c-5-0.3-10.2,1.7-13.8,5.9c-5.3,6.2-5.3,15.5-0.1,21.7c3.6,4.2,8.6,6.2,13.7,6c4.4-0.2,8.7,1.5,11.8,4.6l1.8,1.8
      c3.1,3.1,4.8,7.4,4.6,11.8c-0.2,5,1.8,10.1,6,13.7c6.2,5.2,15.4,5.2,21.6,0c4.2-3.6,6.2-8.6,6-13.7C69.2,83.2,70.8,78.9,73.9,75.8z
      M38.1,39.5l1.6-1.6c3.1-3.1,7.4-4.5,11.8-4.3c0.7,0,1.4,0,2.1,0c4.4-0.3,8.7,1.2,11.8,4.3l1.6,1.6c3.2,3.2,4.7,7.7,4.5,12.3
      c0,0.7,0,1.4,0,2.1c0.3,4.5-1.1,9-4.3,12.2L66.3,67c-3.3,3.3-7.9,4.8-12.6,4.5c-0.7,0-1.5,0-2.2,0c-4.7,0.3-9.3-1.2-12.6-4.5
      l-0.9-0.9c-3.2-3.2-4.6-7.7-4.3-12.2c0.1-0.7,0.1-1.4,0-2.1C33.4,47.2,34.9,42.7,38.1,39.5z"/>
    </svg>
  ),
  home: HomeIcon,
  about: InfoIcon,
  shop: StoreIcon,
  movie: ClapperboardIcon,
  explore: CompassIcon,
  search: SearchIcon,
  feed: ZapIcon,
  eye: EyeIcon,
  addPlaylist: ListPlus,
  playlist: ListVideoIcon,
  movieReview: SignatureIcon,
  watchlist: BookmarkIcon,
  like: HeartIcon,
  rating: StarIcon,
  sparkles: SparklesIcon,
  share: Link,
  delete: Trash,
  close: X,
  spinner: Loader2,
  star: StarIcon,
  logout: LogOutIcon,
  loader: ({ className, ...props }: LucideProps) => (
    <Icons.spinner className={cn('animate-spin', className)} {...props} />
  ),
  legal: ScaleIcon,
  ellipsis: MoreHorizontalIcon,
  comment: TextIcon,
  bell: BellIcon,
  chevronLeft: ChevronLeft,
  chevronRight: ChevronRight,
  trash: Trash,
  post: FileText,
  page: File,
  media: Image,
  settings: Settings,
  billing: CreditCard,
  add: Plus,
  edit: PenIcon,
  lock: LockIcon,
  warning: AlertTriangle,
  user: User,
  users: UsersIcon,
  arrowUp: ArrowUpIcon,
  arrowDown: ArrowDownIcon,
  arrowRight: ArrowRightIcon,
  arrowLeft: ArrowLeftIcon,
  help: HelpCircle,
  pizza: Pizza,
  error: AlertCircleIcon,
  minus: MinusIcon,
  twitter: Twitter,
  archive: ArchiveIcon,
  archiveRestore: ArchiveRestoreIcon,
  check: CheckIcon,
  send: SendIcon,
  copy: CopyIcon,
  copyDone: CheckIcon,
  sun: SunMedium,
  moon: Moon,
  laptop: Laptop,
  filter: FilterIcon,
  X: Cross2Icon,
  orderAsc: ArrowUpNarrowWideIcon,
  orderDesc: ArrowDownNarrowWideIcon,
  clock: ClockIcon,
  gitHub: (props: LucideProps) => (
    <svg viewBox="0 0 438.549 438.549" {...props}>
      <path
        fill="currentColor"
        d="M409.132 114.573c-19.608-33.596-46.205-60.194-79.798-79.8-33.598-19.607-70.277-29.408-110.063-29.408-39.781 0-76.472 9.804-110.063 29.408-33.596 19.605-60.192 46.204-79.8 79.8C9.803 148.168 0 184.854 0 224.63c0 47.78 13.94 90.745 41.827 128.906 27.884 38.164 63.906 64.572 108.063 79.227 5.14.954 8.945.283 11.419-1.996 2.475-2.282 3.711-5.14 3.711-8.562 0-.571-.049-5.708-.144-15.417a2549.81 2549.81 0 01-.144-25.406l-6.567 1.136c-4.187.767-9.469 1.092-15.846 1-6.374-.089-12.991-.757-19.842-1.999-6.854-1.231-13.229-4.086-19.13-8.559-5.898-4.473-10.085-10.328-12.56-17.556l-2.855-6.57c-1.903-4.374-4.899-9.233-8.992-14.559-4.093-5.331-8.232-8.945-12.419-10.848l-1.999-1.431c-1.332-.951-2.568-2.098-3.711-3.429-1.142-1.331-1.997-2.663-2.568-3.997-.572-1.335-.098-2.43 1.427-3.289 1.525-.859 4.281-1.276 8.28-1.276l5.708.853c3.807.763 8.516 3.042 14.133 6.851 5.614 3.806 10.229 8.754 13.846 14.842 4.38 7.806 9.657 13.754 15.846 17.847 6.184 4.093 12.419 6.136 18.699 6.136 6.28 0 11.704-.476 16.274-1.423 4.565-.952 8.848-2.383 12.847-4.285 1.713-12.758 6.377-22.559 13.988-29.41-10.848-1.14-20.601-2.857-29.264-5.14-8.658-2.286-17.605-5.996-26.835-11.14-9.235-5.137-16.896-11.516-22.985-19.126-6.09-7.614-11.088-17.61-14.987-29.979-3.901-12.374-5.852-26.648-5.852-42.826 0-23.035 7.52-42.637 22.557-58.817-7.044-17.318-6.379-36.732 1.997-58.24 5.52-1.715 13.706-.428 24.554 3.853 10.85 4.283 18.794 7.952 23.84 10.994 5.046 3.041 9.089 5.618 12.135 7.708 17.705-4.947 35.976-7.421 54.818-7.421s37.117 2.474 54.823 7.421l10.849-6.849c7.419-4.57 16.18-8.758 26.262-12.565 10.088-3.805 17.802-4.853 23.134-3.138 8.562 21.509 9.325 40.922 2.279 58.24 15.036 16.18 22.559 35.787 22.559 58.817 0 16.178-1.958 30.497-5.853 42.966-3.9 12.471-8.941 22.457-15.125 29.979-6.191 7.521-13.901 13.85-23.131 18.986-9.232 5.14-18.182 8.85-26.84 11.136-8.662 2.286-18.415 4.004-29.263 5.146 9.894 8.562 14.842 22.077 14.842 40.539v60.237c0 3.422 1.19 6.279 3.572 8.562 2.379 2.279 6.136 2.95 11.276 1.995 44.163-14.653 80.185-41.062 108.068-79.226 27.88-38.161 41.825-81.126 41.825-128.906-.01-39.771-9.818-76.454-29.414-110.049z"
      ></path>
    </svg>
  ),
  radix: (props: LucideProps) => (
    <svg viewBox="0 0 25 25" fill="none" {...props}>
      <path
        d="M12 25C7.58173 25 4 21.4183 4 17C4 12.5817 7.58173 9 12 9V25Z"
        fill="currentcolor"
      ></path>
      <path d="M12 0H4V8H12V0Z" fill="currentcolor"></path>
      <path
        d="M17 8C19.2091 8 21 6.20914 21 4C21 1.79086 19.2091 0 17 0C14.7909 0 13 1.79086 13 4C13 6.20914 14.7909 8 17 8Z"
        fill="currentcolor"
      ></path>
    </svg>
  ),
  aria: (props: LucideProps) => (
    <svg role="img" viewBox="0 0 24 24" fill="currentColor" {...props}>
      <path d="M13.966 22.624l-1.69-4.281H8.122l3.892-9.144 5.662 13.425zM8.884 1.376H0v21.248zm15.116 0h-8.884L24 22.624Z" />
    </svg>
  ),
  npm: (props: LucideProps) => (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        d="M1.763 0C.786 0 0 .786 0 1.763v20.474C0 23.214.786 24 1.763 24h20.474c.977 0 1.763-.786 1.763-1.763V1.763C24 .786 23.214 0 22.237 0zM5.13 5.323l13.837.019-.009 13.836h-3.464l.01-10.382h-3.456L12.04 19.17H5.113z"
        fill="currentColor"
      />
    </svg>
  ),
  yarn: (props: LucideProps) => (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        d="M12 0C5.375 0 0 5.375 0 12s5.375 12 12 12 12-5.375 12-12S18.625 0 12 0zm.768 4.105c.183 0 .363.053.525.157.125.083.287.185.755 1.154.31-.088.468-.042.551-.019.204.056.366.19.463.375.477.917.542 2.553.334 3.605-.241 1.232-.755 2.029-1.131 2.576.324.329.778.899 1.117 1.825.278.774.31 1.478.273 2.015a5.51 5.51 0 0 0 .602-.329c.593-.366 1.487-.917 2.553-.931.714-.009 1.269.445 1.353 1.103a1.23 1.23 0 0 1-.945 1.362c-.649.158-.95.278-1.821.843-1.232.797-2.539 1.242-3.012 1.39a1.686 1.686 0 0 1-.704.343c-.737.181-3.266.315-3.466.315h-.046c-.783 0-1.214-.241-1.45-.491-.658.329-1.51.19-2.122-.134a1.078 1.078 0 0 1-.58-1.153 1.243 1.243 0 0 1-.153-.195c-.162-.25-.528-.936-.454-1.946.056-.723.556-1.367.88-1.71a5.522 5.522 0 0 1 .408-2.256c.306-.727.885-1.348 1.32-1.737-.32-.537-.644-1.367-.329-2.21.227-.602.412-.936.82-1.08h-.005c.199-.074.389-.153.486-.259a3.418 3.418 0 0 1 2.298-1.103c.037-.093.079-.185.125-.283.31-.658.639-1.029 1.024-1.168a.94.94 0 0 1 .328-.06zm.006.7c-.507.016-1.001 1.519-1.001 1.519s-1.27-.204-2.266.871c-.199.218-.468.334-.746.44-.079.028-.176.023-.417.672-.371.991.625 2.094.625 2.094s-1.186.839-1.626 1.881c-.486 1.144-.338 2.261-.338 2.261s-.843.732-.899 1.487c-.051.663.139 1.2.343 1.515.227.343.51.176.51.176s-.561.653-.037.931c.477.25 1.283.394 1.71-.037.31-.31.371-1.001.486-1.283.028-.065.12.111.209.199.097.093.264.195.264.195s-.755.324-.445 1.066c.102.246.468.403 1.066.398.222-.005 2.664-.139 3.313-.296.375-.088.505-.283.505-.283s1.566-.431 2.998-1.357c.917-.598 1.293-.76 2.034-.936.612-.148.57-1.098-.241-1.084-.839.009-1.575.44-2.196.825-1.163.718-1.742.672-1.742.672l-.018-.032c-.079-.13.371-1.293-.134-2.678-.547-1.515-1.413-1.881-1.344-1.997.297-.5 1.038-1.297 1.334-2.78.176-.899.13-2.377-.269-3.151-.074-.144-.732.241-.732.241s-.616-1.371-.788-1.483a.271.271 0 0 0-.157-.046z"
        fill="currentColor"
      />
    </svg>
  ),
  pnpm: (props: LucideProps) => (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        d="M0 0v7.5h7.5V0zm8.25 0v7.5h7.498V0zm8.25 0v7.5H24V0zM8.25 8.25v7.5h7.498v-7.5zm8.25 0v7.5H24v-7.5zM0 16.5V24h7.5v-7.5zm8.25 0V24h7.498v-7.5zm8.25 0V24H24v-7.5z"
        fill="currentColor"
      />
    </svg>
  ),
  react: (props: LucideProps) => (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        d="M14.23 12.004a2.236 2.236 0 0 1-2.235 2.236 2.236 2.236 0 0 1-2.236-2.236 2.236 2.236 0 0 1 2.235-2.236 2.236 2.236 0 0 1 2.236 2.236zm2.648-10.69c-1.346 0-3.107.96-4.888 2.622-1.78-1.653-3.542-2.602-4.887-2.602-.41 0-.783.093-1.106.278-1.375.793-1.683 3.264-.973 6.365C1.98 8.917 0 10.42 0 12.004c0 1.59 1.99 3.097 5.043 4.03-.704 3.113-.39 5.588.988 6.38.32.187.69.275 1.102.275 1.345 0 3.107-.96 4.888-2.624 1.78 1.654 3.542 2.603 4.887 2.603.41 0 .783-.09 1.106-.275 1.374-.792 1.683-3.263.973-6.365C22.02 15.096 24 13.59 24 12.004c0-1.59-1.99-3.097-5.043-4.032.704-3.11.39-5.587-.988-6.38-.318-.184-.688-.277-1.092-.278zm-.005 1.09v.006c.225 0 .406.044.558.127.666.382.955 1.835.73 3.704-.054.46-.142.945-.25 1.44-.96-.236-2.006-.417-3.107-.534-.66-.905-1.345-1.727-2.035-2.447 1.592-1.48 3.087-2.292 4.105-2.295zm-9.77.02c1.012 0 2.514.808 4.11 2.28-.686.72-1.37 1.537-2.02 2.442-1.107.117-2.154.298-3.113.538-.112-.49-.195-.964-.254-1.42-.23-1.868.054-3.32.714-3.707.19-.09.4-.127.563-.132zm4.882 3.05c.455.468.91.992 1.36 1.564-.44-.02-.89-.034-1.345-.034-.46 0-.915.01-1.36.034.44-.572.895-1.096 1.345-1.565zM12 8.1c.74 0 1.477.034 2.202.093.406.582.802 1.203 1.183 1.86.372.64.71 1.29 1.018 1.946-.308.655-.646 1.31-1.013 1.95-.38.66-.773 1.288-1.18 1.87-.728.063-1.466.098-2.21.098-.74 0-1.477-.035-2.202-.093-.406-.582-.802-1.204-1.183-1.86-.372-.64-.71-1.29-1.018-1.946.303-.657.646-1.313 1.013-1.954.38-.66.773-1.286 1.18-1.868.728-.064 1.466-.098 2.21-.098zm-3.635.254c-.24.377-.48.763-.704 1.16-.225.39-.435.782-.635 1.174-.265-.656-.49-1.31-.676-1.947.64-.15 1.315-.283 2.015-.386zm7.26 0c.695.103 1.365.23 2.006.387-.18.632-.405 1.282-.66 1.933-.2-.39-.41-.783-.64-1.174-.225-.392-.465-.774-.705-1.146zm3.063.675c.484.15.944.317 1.375.498 1.732.74 2.852 1.708 2.852 2.476-.005.768-1.125 1.74-2.857 2.475-.42.18-.88.342-1.355.493-.28-.958-.646-1.956-1.1-2.98.45-1.017.81-2.01 1.085-2.964zm-13.395.004c.278.96.645 1.957 1.1 2.98-.45 1.017-.812 2.01-1.086 2.964-.484-.15-.944-.318-1.37-.5-1.732-.737-2.852-1.706-2.852-2.474 0-.768 1.12-1.742 2.852-2.476.42-.18.88-.342 1.356-.494zm11.678 4.28c.265.657.49 1.312.676 1.948-.64.157-1.316.29-2.016.39.24-.375.48-.762.705-1.158.225-.39.435-.788.636-1.18zm-9.945.02c.2.392.41.783.64 1.175.23.39.465.772.705 1.143-.695-.102-1.365-.23-2.006-.386.18-.63.406-1.282.66-1.933zM17.92 16.32c.112.493.2.968.254 1.423.23 1.868-.054 3.32-.714 3.708-.147.09-.338.128-.563.128-1.012 0-2.514-.807-4.11-2.28.686-.72 1.37-1.536 2.02-2.44 1.107-.118 2.154-.3 3.113-.54zm-11.83.01c.96.234 2.006.415 3.107.532.66.905 1.345 1.727 2.035 2.446-1.595 1.483-3.092 2.295-4.11 2.295-.22-.005-.406-.05-.553-.132-.666-.38-.955-1.834-.73-3.703.054-.46.142-.944.25-1.438zm4.56.64c.44.02.89.034 1.345.034.46 0 .915-.01 1.36-.034-.44.572-.895 1.095-1.345 1.565-.455-.47-.91-.993-1.36-1.565z"
        fill="currentColor"
      />
    </svg>
  ),
  tailwind: (props: LucideProps) => (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        d="M12.001,4.8c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 C13.666,10.618,15.027,12,18.001,12c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C16.337,6.182,14.976,4.8,12.001,4.8z M6.001,12c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 c1.177,1.194,2.538,2.576,5.512,2.576c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C10.337,13.382,8.976,12,6.001,12z"
        fill="currentColor"
      />
    </svg>
  ),
  google: (props: LucideProps) => (
    <svg role="img" viewBox="0 0 24 24" {...props}>
      <path
        fill="currentColor"
        d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z"
      />
    </svg>
  ),
  apple: (props: LucideProps) => (
    <svg role="img" viewBox="0 0 24 24" {...props}>
      <path
        d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.559-1.701"
        fill="currentColor"
      />
    </svg>
  ),
  paypal: (props: LucideProps) => (
    <svg role="img" viewBox="0 0 24 24" {...props}>
      <path
        d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74L4.944.901C5.026.382 5.474 0 5.998 0h7.46c2.57 0 4.578.543 5.69 1.81 1.01 1.15 1.304 2.42 1.012 4.287-.023.143-.047.288-.077.437-.983 5.05-4.349 6.797-8.647 6.797h-2.19c-.524 0-.968.382-1.05.9l-1.12 7.106zm14.146-14.42a3.35 3.35 0 0 0-.607-.541c-.013.076-.026.175-.041.254-.93 4.778-4.005 7.201-9.138 7.201h-2.19a.563.563 0 0 0-.556.479l-1.187 7.527h-.506l-.24 1.516a.56.56 0 0 0 .554.647h3.882c.46 0 .85-.334.922-.788.06-.26.76-4.852.816-5.09a.932.932 0 0 1 .923-.788h.58c3.76 0 6.705-1.528 7.565-5.946.36-1.847.174-3.388-.777-4.471z"
        fill="currentColor"
      />
    </svg>
  ),
};
