        ...
        {season.episodes?.map((episode, i) => (
          <Card key={i} className="@container/episode-card flex flex-row items-center gap-2 p-2 hover:bg-muted-hover hover:cursor-pointer">
            ...
            <div className='space-y-2'>
              ...
              <h4 className='text-sm text-muted-foreground'>{`${upperFirst(t('common.messages.first_air_date'))} : ${episode.air_date ? formatter.dateTime(new Date(episode.air_date), { year: 'numeric', month: 'long', day: 'numeric' }) : upperFirst(t('common.messages.unknown'))}`}</h4>
            </div>
          </Card>
        ))}
      </div>
