...
export default async function About({
  ...
  return (
    <main className='flex flex-col gap-4 items-center text-justify'>
        ...
        <section id='team' className='w-full flex flex-col gap-4 px-4 max-w-xl'>
            ...
            <p>
              {t.rich('team.last', {
                link: (chunks) => <Link href='https://discord.gg/z4fXr39xPr' target='_blank'>{chunks}</Link>
              })}
            </p>
        </section>
        {/* PRICING */}
        ...
        <section id='data' className='w-full flex flex-col gap-4 px-4 max-w-xl'>
            ...
            <p>
              {t.rich('data.paragraph.1', {
                link: (chunks) => <Link href='https://www.themoviedb.org/' target='_blank'>{chunks}</Link>
              })}
            </p>
            <p>
        ...
        <section id='ressources' className='w-full flex flex-col gap-4 px-4 max-w-xl'>
            ...
            <ul className='space-y-2 list-disc'>
              {ressources.map((ressource, index) => (
                <li key={index}>
                  <Link href={ressource.link} target='_blank'>{ressource.name}</Link>
                </li>
              ))}
            </ul>
        </section>
        ...
        <section id='contact-support' className='w-full flex flex-col gap-4 px-4 max-w-xl'>
            ...
            <p>
              {t.rich('contact-support.help-center', {
                link: (chunks) => <Link href='https://help.recomend.app/' target='_blank'>{chunks}</Link>
              })}
            </p>
            <p>
              {t.rich('contact-support.technical-support', {
                link: (chunks) => <Link href='mailto:help@recomend.app'>{chunks}</Link>
              })}
            </p>
            <p>
              {t.rich('contact-support.suggest-a-feature', {
                link: (chunks) => <Link href='mailto:ideas@recomend.app'>{chunks}</Link>
              })}
            </p>
            <p>
              {t.rich('contact-support.conact-us', {
                link: (chunks) => <Link href='mailto:contact@recomend.app'>{chunks}</Link>
              })}
            </p>
        </section>
        {/* QUOTE */}
        <section id='quote' className='w-full flex flex-col gap-4 px-4 max-w-xl mt-5 py-8'>
          <blockquote className="relative">
            <svg className="absolute -top-6 -start-8 h-16 w-16 text-muted" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <div className="relative">
              ...
              <p className='text-right text-accent-1'>
