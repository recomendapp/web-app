...
export default function MapPopupFullScreen(props: any) {
  ...
  useEffect(() => {
    popupFullScreenRef.current && (setMenuPosition(popupFullScreenRef.current?.clientHeight - 330))
  }, [])


  const autoClose = (event: MouseEvent) => {
    // DETECTE CLICK OUTSIDE SEARBAR
    if (popupFullScreenRef.current && !popupFullScreenRef.current.contains(event.target as Node)) {
  ...
  function handleMouseMove(e: any) {
    ...
    if (popupFullScreenRef.current && (menuPositionOnDown+newPosition) >= 0 && (menuPositionOnDown+newPosition) <= (popupFullScreenRef.current?.clientHeight - 100)) {
        setMenuPosition(menuPositionOnDown + newPosition)
    }
    if (popupFullScreenRef.current && (menuPositionOnDown+newPosition) >= (popupFullScreenRef.current?.clientHeight - 30)) {
      setPopupIsOpen(false)
      onClose()
    }
  }
        ...
        >
            ...
            <div className="px-3 w-full">
              ...
              <div className="grid grid-cols-2 gap-4 w-full ">
                {/* POSTER */}
                <div className="">
                  <Image src={"https://image.tmdb.org/t/p/w500/"+selectedMovie.poster_path} alt={selectedMovie.title} width={100} height={100} className='rounded-md w-full object-cover' />
                </div>
                {/* MAIN DETAILS */}
                <div className="flex flex-col my-2">
                  ...
                  <div className="mt-2">
                    {selectedMovie.crew.filter((person: any) => person.job === 'Director').map((director: any) => director.name).join(', ')}
                  </div>
                </div>
              </div>
              {/* OVERVIEW */}
